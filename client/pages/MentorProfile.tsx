    ...
    <section className="container py-10 md:py-16 grid gap-8 lg:grid-cols-[1fr_360px]">
      <div>
        <div className="flex flex-wrap items-center gap-3">
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight">{m.name}</h1>
          <Badge className="inline-flex items-center gap-1"><BadgeCheck className="size-3" /> Verified</Badge>
        </div>
        <p className="text-muted-foreground">{m.role}{m.company ? ` @ ${m.company}` : ""} • {m.years} yrs • <span className="inline-flex items-center gap-1"><Stars value={m.rating} />{m.rating.toFixed(1)}</span></p>
        ...
        <Tabs defaultValue="overview" className="mt-8">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  ...
                  <ul className="list-disc pl-5 text-sm text-muted-foreground space-y-1">
                    {m.specialties.map((s) => (<li key={s}>{s}</li>))}
                  </ul>
                </div>
                <div>
                  <h3 className="font-semibold mb-2">Outcomes</h3>
                  <ul className="list-disc pl-5 text-sm text-muted-foreground space-y-1">
                    {m.outcomes.map((s) => (<li key={s}>{s}</li>))}
                  </ul>
                </div>
              </div>
              <div className="mt-4">
                ...
                <ul className="list-disc pl-5 text-sm text-muted-foreground space-y-1">
                  {m.sampleQuestions.map((s) => (<li key={s}>{s}</li>))}
                </ul>
              </div>
            </CardContent></Card>
          </TabsContent>
          ...
          <TabsContent value="availability">
            <Card><CardContent className="p-6">
              <p className="text-sm text-muted-foreground">Calendar integration coming soon. For now, book via Contact page or mentor-provided Calendly after chat.</p>
              <div className="mt-4 grid grid-cols-3 gap-3 text-center">
                <div className="rounded-lg bg-success/10 p-3">Weekdays</div>
                <div className="rounded-lg bg-primary/10 p-3">Evenings</div>
                <div className="rounded-lg bg-warning/10 p-3">Weekend slots</div>
              </div>
            </CardContent></Card>
          </TabsContent>
          ...
          <TabsContent value="reviews" className="space-y-4">
            {m.reviews.map((r, i) => (
              <Card key={i}><CardContent className="p-6">
                <div className="flex items-center gap-2"><Stars value={r.rating} /> <span className="text-sm font-medium">{r.name}</span></div>
          ...
          <TabsContent value="policies">
            <Card><CardContent className="p-6">
              <ul className="list-disc pl-5 text-sm text-muted-foreground space-y-1">
                {m.policies.map((p) => (<li key={p}>{p}</li>))}
              </ul>
            </CardContent></Card>
          </TabsContent>
        </Tabs>
